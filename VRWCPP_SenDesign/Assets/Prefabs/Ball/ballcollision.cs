...
public class ballcollision : MonoBehaviour
{
    ...
    private float ballLifeTimer = 0f;
    private float maxBallLifeTime = 5f; // Maximum time the ball can bounce around before ending the round
    private bool hasBeenHitAfterServe = false;
    private float tutorialBallLifetime = 10f; // Time before tutorial balls are automatically destroyed
    ...
    void Update()
    {
        ...
        // Only increment timer if the ball hasn't been hit after serve
        if (!hasBeenHitAfterServe)
        {
            ballLifeTimer += Time.deltaTime;
            
            // Check if the ball has been alive for too long
            if (ballLifeTimer >= maxBallLifeTime)
            {
                // End the round and change serving
                if (!turretball)
                {
                    bool ifAIServe = this.GetComponent<Positiondetection>().teamserve;
                    bool AIhitlast = this.GetComponent<Positiondetection>().whohitlast;
                    
                    // Give point to the opposite team of whoever hit last
                    if (AIhitlast)
                    {
        ...
        // For tutorial balls, destroy them after tutorialBallLifetime seconds
        if (turretball)
        {
            ballLifeTimer += Time.deltaTime;
            if (ballLifeTimer >= tutorialBallLifetime)
            {
                Destroy(gameObject);
            }
        }
    ...
    {
        // Reset timer when ball hits court or paddle
        if (coll.gameObject.name == "Court" || coll.gameObject.name == "ClassicRacket")
        {
            ballLifeTimer = 0f;
        }
        ...
        if (!hitgroundtoorecently && coll.gameObject.name == "Court")
        {
            ...
            /*if (bounces > 2)
            {
                ...
                ifAIServe = this.GetComponent<Positiondetection>().teamserve;
        ...
        if (coll.gameObject.name == "ClassicRacket") //"PicklePaddle_Final"
        {
            hasBeenHitAfterServe = true;
            bool ifAIServe = this.GetComponent<Positiondetection>().teamserve;
            ...
            {
                // Only enforce this rule when it's the player's serve
                // If the AI is serving, allow the player to hit the ball even if it hasn't bounced
                if (!ifAIServe)
                {
            ...
            if (!hitAIrecently)
            {
                if (isservesequence)
                {
                    ...
                    ballrigid.AddForce(direction * 10f, ForceMode.Impulse);
